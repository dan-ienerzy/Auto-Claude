...
import type { CodexVersionInfo, CodexInstallationInfo } from "../../shared/types/cli";
...
export function CodexStatusBadge({ className }: CodexStatusBadgeProps) {
  ...
  const [installations, setInstallations] = useState<CodexInstallationInfo[]>([]);
  ...
  const fetchVersions = useCallback(async () => {
    if (!window.electronAPI?.getCodexVersions) {
      return;
    }
    ...
    try {
      const result = await window.electronAPI.getCodexVersions();
      if (result.success && result.data) {
  ...
  const fetchInstallations = useCallback(async () => {
    if (!window.electronAPI?.getCodexInstallations) {
      return;
    }
    ...
    try {
      const result = await window.electronAPI.getCodexInstallations();
      if (result.success && result.data) {
  ...
  // Fetch versions when popover opens and Codex is installed
  useEffect(() => {
  ...
  const performVersionSwitch = async () => {
    ...
    try {
      if (!window.electronAPI?.installCodexVersion) {
        setInstallError("Version switching not available");
        return;
      }

      const result = await window.electronAPI.installCodexVersion(selectedVersion);
      ...
      console.error("Failed to switch Codex version:", err);
  ...
  const performPathSwitch = async () => {
    ...
    try {
      if (!window.electronAPI?.setCodexActivePath) {
        setInstallError("Path switching not available");
        return;
      }

      const result = await window.electronAPI.setCodexActivePath(selectedInstallation);
      ...
      console.error("Failed to switch Codex CLI path:", err);
    ...
    <Popover open={isOpen} onOpenChange={setIsOpen}>
        ...
        <TooltipTrigger asChild>
          <PopoverTrigger asChild>
            ...
            >
              ...
              <span className="truncate">Codex CLI</span>
              {status === "outdated" && (
      ...
      <PopoverContent side="right" align="end" className="w-72">
        <div className="space-y-3">
          ...
          {/* Version selector - only show when Codex is installed */}
          {versionInfo?.installed && (
            <div className="space-y-1.5">
              <label className="text-xs text-muted-foreground">
                {t("navigation:codex.switchVersion", "Switch Version")}
              </label>
              <Select
              ...
              >
                <SelectTrigger className="h-8 text-xs">
                  <SelectValue
                    placeholder={
                      isLoadingVersions
                        ? t("navigation:codex.loadingVersions", "Loading versions...")
                        : versionsError
                          ? t("navigation:codex.failedToLoadVersions", "Failed to load versions")
                          : t("navigation:codex.selectVersion", "Select version")
                    }
                  />
                </SelectTrigger>
                <SelectContent>
                      ...
                      >
                        ...
                        {isCurrentVersion && (
                          <span className="ml-2 text-muted-foreground">
                            ({t("navigation:codex.currentVersion", "Current")})
                          </span>
                        )}
                      </SelectItem>
                    );
          ...
          {installations.length > 1 && (
            <div className="space-y-1.5">
              <label className="text-xs text-muted-foreground">
                {t("navigation:codex.switchInstallation", "Switch Installation")}
              </label>
              <Select
                  ...
                  <SelectValue
                    placeholder={
                      isLoadingInstallations
                        ? t("navigation:codex.loadingInstallations", "Loading installations...")
                        : installationsError
                          ? t("navigation:codex.failedToLoadInstallations", "Failed to load installations")
                          : t("navigation:codex.selectInstallation", "Select installation")
                    }
                  />
                </SelectTrigger>
                <SelectContent>
                    ...
                    >
                      <div className="flex flex-col">
                        ...
                        <span className="text-muted-foreground text-[9px]">
                          {installation.version ? `v${installation.version}` : t("navigation:codex.versionUnknown", "version unknown")} ({installation.source})
                          {installation.isActive && ` - ${t("navigation:codex.activeInstallation", "Active")}`}
                        </span>
                      </div>
                    </SelectItem>
                  ))}
          ...
          <Button
            ...
            aria-label={t(
              "navigation:codex.learnMoreAriaLabel",
              "Learn more about Codex CLI (opens in new window)"
            )}
          >
            {t("navigation:codex.learnMore", "Learn more about Codex CLI")}
          ...
          <Button
            ...
            aria-label={t(
              "navigation:codex.viewChangelogAriaLabel",
              "View Codex Changelog (opens in new window)"
            )}
          >
            {t("navigation:codex.viewChangelog", "View Codex Changelog")}
            <ExternalLink className="h-3 w-3" aria-hidden="true" />
          </Button>
        </div>
      </PopoverContent>
      ...
      <AlertDialog open={showUpdateWarning} onOpenChange={setShowUpdateWarning}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {t(
                "navigation:codex.updateWarningDescription",
                "Updating will close all running Codex sessions. Any unsaved work in those sessions may be lost. Make sure to save your work before proceeding."
              )}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
      ...
      <AlertDialog open={showRollbackWarning} onOpenChange={setShowRollbackWarning}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              {t("navigation:codex.rollbackWarningTitle", "Switch to version {{version}}?", {
                version: selectedVersion,
              })}
            </AlertDialogTitle>
            <AlertDialogDescription>
              {t(
                "navigation:codex.rollbackWarningDescription",
                "Switching versions will close all running Codex sessions. Any unsaved work in those sessions may be lost. Make sure to save your work before proceeding."
              )}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={performVersionSwitch}>
              {t("navigation:codex.switchAnyway", "Switch Anyway")}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
      ...
      <AlertDialog open={showPathChangeWarning} onOpenChange={setShowPathChangeWarning}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              {t("navigation:codex.pathChangeWarningTitle", "Switch CLI installation?")}
            </AlertDialogTitle>
            <AlertDialogDescription>
              {t(
                "navigation:codex.pathChangeWarningDescription",
                "Switching CLI installations will use a different Codex binary. Any running sessions will continue using the previous installation until restarted."
              )}
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={performPathSwitch}>
              {t("navigation:codex.switchInstallationConfirm", "Switch")}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
